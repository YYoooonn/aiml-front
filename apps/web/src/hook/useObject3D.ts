...
import { createObject3D, deleteObject3D, updateObject3D } from "@/app/actions/object3d";
...
export const useObject3D = () => {
  ...
  const selectObject3D = (data: TObject3DData, isNew=false) => {
    if (isNew) setSelected({ [data.id]: data });
      ...
      if (data.parentId !== object3DInfo.parentId) updated = {[data.id]: data}
  ...
  const saveObject3D = async (data: TObject3D & { id?: string }) => {
    ...
    const isPresent = data.id && selectedScene.children.some((child) => child.id === data.id);
    const response = isPresent
      ? await updateObject3D({ ...data, sceneId: selectedScene.id })
      : await createObject3D({ ...data, sceneId: selectedScene.id, id: undefined });
  ...
  const saveSelected = async () => {
    ...
    if (selectedData.length !== 1) return { success: false, error: "not implemented yet" }
